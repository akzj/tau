...
func init() {
	GlobalSkills.Register(&SkillDefinition{
		Name:        "security_audit",
		Description: "Audit codebase for security vulnerabilities: secrets, injection, unsafe dependencies",
		Tools:       []string{"search_code", "find_references", "lint_code", "env_manage", "read", "git_log"},
		Prompt:      securityAuditPrompt,
		Category:    "security",
	})
}

const securityAuditPrompt = `Security audit checklist:
