...
// Compat is a sealed interface for wire-specific compatibility transforms.
...
func (r *ProviderRegistry) RegisterVendor(name string, cfg VendorConfig) error {
	...
	if cfg.WireAPI == "" {
		return &TauError{Code: ErrSession, Message: "vendor must specify a WireAPI"}
	}
	r.vendors[name] = cfg
	if _, exists := r.defaults[cfg.WireAPI]; !exists {
		r.defaults[cfg.WireAPI] = name
	}
	return nil
}
