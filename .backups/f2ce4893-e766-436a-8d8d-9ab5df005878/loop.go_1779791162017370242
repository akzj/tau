...
func (l *Loop) Prompt(ctx context.Context, sess *Session, input UserInput) (*Run, error) {
	...
	// 4. Call provider
	provEvents, err := sess.Provider.Stream(ctx, req)
	if err != nil {
...
func (r *Run) processProviderEvents(ctx context.Context, provEvents <-chan ProviderEvent, req StreamRequest) {
	...
	for pe := range provEvents {
		...
		case ProvToolCallEnd:
			...
			// Execute tool if we have it registered
			if tool, ok := r.sess.Tools.Get(acc.name); ok {
				result, err := tool.Execute(ctx, pe.ToolCallID, json.RawMessage(argsJSON), func(pr PartialResult) {})
				if err != nil {
					r.events <- ErrorEvent{Timestamp_: timeNow(), Err: err, Code: ErrTool}
				}
				...
				r.sess.Transcript.Append(toolMsg)
			}
