...
// Azure is OpenAI-compatible but uses api-key header and endpoint-based URLs.
...
// NewProvider creates an Azure OpenAI provider from environment variables.
//
// Env vars:
//
...
func (p *Provider) Complete(ctx context.Context, req core.CompleteRequest) (core.CompleteResponse, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return core.CompleteResponse{}, p.classifyError("azure.Complete", resp.StatusCode, bodyBytes)
	}
...
func (p *Provider) Stream(ctx context.Context, req core.StreamRequest) (<-chan core.ProviderEvent, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, p.classifyError("azure.Stream", resp.StatusCode, bodyBytes)
	}
...
// buildRequest constructs the Azure OpenAI Chat Completions API request body.
...
// newRequest creates an HTTP request with Azure OpenAI headers.
...
// azureSSEChunk is the JSON payload of an Azure OpenAI SSE data line.
type azureSSEChunk struct {
	ID      string `json:"id"`
...
// parseSSE reads Azure OpenAI SSE streaming responses and emits ProviderEvents.
