...
func TestNewProviderEnv(t *testing.T) {
	...
	_, err := NewProvider()
	if err == nil {
		t.Error("expected error for missing DEEPSEEK_API_KEY")
	}
	...
	if prov.baseURL != "https://api.deepseek.com/v1" {
		t.Errorf("expected baseURL https://api.deepseek.com/v1, got %q", prov.baseURL)
	}
}
...
func TestCompleteText(t *testing.T) {
	...
	resp, err := prov.Complete(context.Background(), core.CompleteRequest{
		Model:        core.ModelSpec{Name: "deepseek-chat"},
...
data: [DONE]`
	...
	prov := newTestProvider(srv)
	ch, err := prov.Stream(context.Background(), core.StreamRequest{Model: core.ModelSpec{Name: "deepseek-chat"}})
	if err != nil {
		t.Fatal(err)
	}
...
data: {"id":"chatcmpl-002","choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"function":{"arguments":"{\"city\":\"Paris\"}"}}]}}]}
