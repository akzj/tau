...
// Provider implements core.Provider for DeepSeek (OpenAI-compatible).
...
// NewProvider creates a DeepSeek provider from environment variables.
//
// Env vars:
//
...
func (p *Provider) Complete(ctx context.Context, req core.CompleteRequest) (core.CompleteResponse, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return core.CompleteResponse{}, p.classifyError("deepseek.Complete", resp.StatusCode, bodyBytes)
	}
...
func (p *Provider) Stream(ctx context.Context, req core.StreamRequest) (<-chan core.ProviderEvent, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, p.classifyError("deepseek.Stream", resp.StatusCode, bodyBytes)
	}
...
// buildRequest constructs the DeepSeek Chat Completions API request body.
...
// newRequest creates an HTTP request with DeepSeek headers.
...
// deepseekSSEChunk is the JSON payload of a DeepSeek SSE data line.
...
// parseSSE reads DeepSeek SSE streaming responses and emits ProviderEvents.
