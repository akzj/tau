...
func TestNewProviderEnv(t *testing.T) {
	...
	_, err := NewProvider()
	if err == nil {
		t.Error("expected error for missing TOGETHER_API_KEY")
	}
	...
	if prov.baseURL != "https://api.together.xyz/v1" {
...
func TestCompleteText(t *testing.T) {
	...
	resp, err := prov.Complete(context.Background(), core.CompleteRequest{
		Model:        core.ModelSpec{Name: "mistralai/Mixtral-8x7B-Instruct-v0.1"},
...
data: [DONE]`
	...
	prov := newTestProvider(srv)
	ch, err := prov.Stream(context.Background(), core.StreamRequest{Model: core.ModelSpec{Name: "mistralai/Mixtral-8x7B-Instruct-v0.1"}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestVisionImage(t *testing.T) {
	...
	prov := newTestProvider(srv)
	resp, err := prov.Complete(context.Background(), core.CompleteRequest{
		Model:    core.ModelSpec{Name: "meta-llama/Llama-3.2-90B-Vision-Instruct-Turbo"},
