...
// Provider implements core.Provider for Together AI (OpenAI-compatible).
// Together uses Bearer token auth and a simple base URL.
...
// NewProvider creates a Together AI provider from environment variables.
//
// Env vars:
//
...
func (p *Provider) Complete(ctx context.Context, req core.CompleteRequest) (core.CompleteResponse, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return core.CompleteResponse{}, p.classifyError("together.Complete", resp.StatusCode, bodyBytes)
	}
...
func (p *Provider) Stream(ctx context.Context, req core.StreamRequest) (<-chan core.ProviderEvent, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, p.classifyError("together.Stream", resp.StatusCode, bodyBytes)
	}
...
// buildRequest constructs the Together AI Chat Completions API request body.
...
// newRequest creates an HTTP request with Together AI Bearer token auth.
...
// togetherSSEChunk is the JSON payload of a Together AI SSE data line.
type togetherSSEChunk struct {
	ID      string `json:"id"`
...
// parseSSE reads Together AI SSE streaming responses and emits ProviderEvents.
