...
// Provider implements core.Provider for the OpenAI Chat Completions API.
//
// Requirements:
//  1. Full POST /v1/chat/completions: model, max_tokens, messages, system, tools, tool_choice, stop, stream.
//  2. SSE streaming via data: {...} → core.ProviderEvent channel.
//  3. Vision: image_url content blocks with base64 data URIs.
//  4. Tool calls: type:function tools[] → tool_calls in response.
//  5. Token usage: usage{} field parsed from complete + streaming.
//  6. Error classification: 429→Transient, 400→Usage, 401/403→Permanent, 500+→Transient.
//  7. Backward compat: Compat() returns core.OpenAICompletionsCompat.
...
func (p *Provider) Complete(ctx context.Context, req core.CompleteRequest) (core.CompleteResponse, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return core.CompleteResponse{}, p.classifyError("openai.Complete", resp.StatusCode, bodyBytes)
	}
...
func (p *Provider) Stream(ctx context.Context, req core.StreamRequest) (<-chan core.ProviderEvent, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, p.classifyError("openai.Stream", resp.StatusCode, bodyBytes)
	}
...
// buildRequest constructs the OpenAI Chat Completions API request body.
//
// Requirements:
//  1. Full POST /v1/chat/completions: model, max_tokens, messages, stream.
//  3. Vision: image_url content blocks with base64 data URIs.
//  4. Tool calls: tools[] with type:function + tool_choice: "auto".
...
// newRequest creates an HTTP request with required OpenAI headers.
func (p *Provider) newRequest(ctx context.Context, body map[string]any) (*http.Request, error) {
	...
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost,
		p.baseURL, bytes.NewReader(jsonBody))
	if err != nil {
...
// openaiSSEChunk is the JSON payload of an OpenAI SSE data line.
...
// parseSSE reads OpenAI SSE streaming responses and emits ProviderEvents.
func (p *Provider) parseSSE(ctx context.Context, body io.ReadCloser, ch chan<- core.ProviderEvent) {
	...
	for scanner.Scan() {
		...
		// Emit usage if present (requirement 5: token usage).
		if chunk.Usage != nil {
		...
		for _, choice := range chunk.Choices {
			...
			if choice.FinishReason != nil {
				...
				// Note: MessageEnd is emitted on [DONE] or final finish_reason.
			}
		}
	}
