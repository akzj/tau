...
// Provider implements core.Provider for Groq (OpenAI-compatible).
// Groq uses Bearer token auth with a flat base URL.
...
// NewProvider creates a Groq provider from environment variables.
//
// Env vars:
//
...
func (p *Provider) Complete(ctx context.Context, req core.CompleteRequest) (core.CompleteResponse, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return core.CompleteResponse{}, p.classifyError("groq.Complete", resp.StatusCode, bodyBytes)
	}
...
func (p *Provider) Stream(ctx context.Context, req core.StreamRequest) (<-chan core.ProviderEvent, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, p.classifyError("groq.Stream", resp.StatusCode, bodyBytes)
	}
...
// buildRequest constructs the Groq Chat Completions API request body.
...
// newRequest creates an HTTP request with Groq headers.
// Groq uses Bearer token auth like OpenAI.
...
// groqSSEChunk is the JSON payload of a Groq SSE data line.
...
// parseSSE reads Groq SSE streaming responses and emits ProviderEvents.
