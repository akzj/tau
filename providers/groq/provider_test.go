...
func TestNewProviderEnv(t *testing.T) {
	...
	_, err := NewProvider()
	if err == nil {
		t.Error("expected error for missing GROQ_API_KEY")
	}
	...
	if prov.baseURL != groqBaseURL {
		t.Errorf("expected baseURL %s, got %q", groqBaseURL, prov.baseURL)
	}

	// Custom base URL.
	os.Setenv("GROQ_BASE_URL", "https://custom.groq.example.com/openai/v1")
	prov, err = NewProvider()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if prov.baseURL != "https://custom.groq.example.com/openai/v1" {
		t.Errorf("expected custom baseURL, got %q", prov.baseURL)
	}
}
